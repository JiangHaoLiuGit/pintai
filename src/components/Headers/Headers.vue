  ...
  <div class="headers">
    ...
    <div class="headers-right">
      <div class="headers-right-left">
        ...
        <el-tooltip
          class="item"
          style="margin: 0 20px 0 10px; color: #171700"
          effect="dark"
          content="主题"
          placement="bottom"
        >
      ...
      <div class="headers-right-right">
        ...
        <el-dropdown size="medium" @command="handleCommand">
          <span class="el-dropdown-link">
            {{admin_account}}<i class="el-icon-arrow-down el-icon--right"></i>
          </span>
          <el-dropdown-menu slot="dropdown">
            <el-dropdown-item command="password_change">密码修改</el-dropdown-item>
...
import { quit , save_pwd_ht } from "../../utils/axios";
// 全屏组件
...
export default {
  ...
  data() {
    return {
      squareUrl: JSON.parse(localStorage.getItem('admin_message')).logo,
      isFullscreen: false,
  ...
  methods: {
    ...
    // 自定义 切换 侧边栏 事件
    targetIcon() {
      this.$store.commit('SET_SHOW',!this.asideShow)
      ...
      switch (command) {
        case "password_change":
          this.$prompt('请输入密码', '修改密码', {
            ...
            inputErrorMessage: '密码不能包含中文和特殊字符'
          }).then(({ value }) => {
            if(value.length < 9 || value.length > 40) {
              ...
              save_pwd_ht({
                pwd : value,
                id: JSON.parse(localStorage.getItem('admin_message')).id
              })
                .then((res) => {
                  console.log(res);
                  if (res.data.err_code == 0) {
                    ...
                    let ttt = setTimeout(() => {
                      clearTimeout(ttt)
                      let src = that.$store.dispatch("user/resetToken");
