                ...
                <div class="user" v-else>
                  ...
                  <div class="content clearfix">
                    ...
                    <div class="news" style="width: 500px" v-else-if="item.type == 5 || item.type == 'order'">
                      ...
                      <!-- 2.0bug 以后会调整 -->
                      <!-- <div class="flexBet" style="width: 339px;height:119px;padding:8px;">
                        <div style="width:100px;height:100px;" class="flexCen">
                          <el-image
        ...
        <div class="bottom">
          ...
          <el-collapse accordion>
            <el-collapse-item>
              ...
              <div class="divS" v-for="(item,index) in useful.greetings" :key="index" @click="fulSend(item)">{{item}}</div>
            </el-collapse-item>
            <el-collapse-item>
              <template slot="title">
                  售后用语
              </template>
              <div class="divS" v-for="(item,index) in useful.after_sale" :key="index" @click="fulSend(item)">{{item}}</div>
            </el-collapse-item>
          </el-collapse>
        </div>
      </div>
...
*/
      ...
      if(this.pick_condition === 0){
        ...
        if(arr.indexOf("<p>") != -1){
          ...
          for(let i = 0 ; i < arr.length ; i ++){
            arr[i] = arr[i].split("</p>")[0]
            if(arr[i].indexOf('">') != -1){
              let newArr = arr[i].split('">')
              arr[i] = newArr[1]
              let img = newArr[0]
              arrImg.push(img.split('src="')[1])
            }
          }
        }
        arr.forEach(item=>{
    ...
    uploadToServer(file, callback) {
        ...
        let fd = new FormData()
        fd.append('file', file)
        xhr.open('post', publicFile.address+"/new_admin.php?c=public&a=upload")
      ...
      uploadAttachment(response, file, fileList) {
        ...
        if (response.err_code === 0) {
          ...
          quill.insertEmbed(length, 'image', response.err_msg.url)
