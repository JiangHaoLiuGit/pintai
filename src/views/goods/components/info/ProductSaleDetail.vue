    ...
    >
      <div v-for="item,index in value.param_group_list" :key="index">
        ...
        >
          ...
          <!-- 单选 -->
          <el-radio-group v-model="item_first.final_value" v-show="item_first.type == 1" disabled>
              <el-radio v-for="(item_second,index_second) in item_first.array" :key="index_second" disabled :label="index_second" >{{item_second}}{{item_first.unit}}</el-radio>
          </el-radio-group>

          <!-- 下拉列表框 -->
          <el-select v-model="item_first.final_value" :placeholder="`请选择${item_first.name}`" disabled v-show="item_first.type == 2">
            <el-option
          ...
          <!-- 多选框 -->
          <el-checkbox-group v-model="item_first.selectValue" v-show="item_first.type == 5" disabled>
  ...
  export default {
    ...
    methods: {
      ...
      handleNext() {
        ...
        console.log(this.value.enableTemplate)
        that.$emit('nextStep')
      }
    }
  }
