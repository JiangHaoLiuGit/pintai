        ...
        <el-row :gutter="20" class="goodsindex-list">
          <el-col :span="24">
            ...
            >
              ...
              <el-table-column label="限制数量">
                  <template slot-scope="scope">
                    <el-input
                      placeholder="请输入备注"
                      v-model="scope.row.limit_quota_num"
                      @focus="inputf(scope.row.limit_quota_num)"
                      @blur="inputs(scope.row)"
                      style="width:100%"
                    >
                    </el-input>
                    <!-- <div>{{scope.row.limit_quota_num}}</div> -->
                </template>
              </el-table-column>
              <el-table-column label="级别">
                <template slot-scope="scope">
                  <div v-if="scope.row.level == 1">省</div>
                  <div v-else-if="scope.row.level == 2">市</div>
                  <div v-else>未知</div>
                </template>
              </el-table-column>
            </el-table>
...
<script>
import { selling_area,area_edit } from "../../utils/axios";
export default {
  data() {
    return {
      queryInfo: {
      ...
      edit_tan: false,
      add_tan: false,
      fn1: this.commonJs.Debounce(this.get_selling_area),
      extendList: [],
      seachVale: "",
      loading: false,
      multipleSelection: [],
  ...
  methods: {
    ...
    inputs(val){
      let data = {
        limit_quota_num:val.limit_quota_num,
        id:val.id
      }
      if(this.inputFirst == val.limit_quota_num){
        return
      }
      area_edit(
        data
