      ...
      <div class="textItem">
        <div class="clearfix" style="margin-bottom: 20px">
          <el-input
            ...
            style="width: 300px"
            placeholder="请输入用户昵称/用户id/用户账号"
            prefix-icon="el-icon-search"
            @keyup.enter.native="pick_seach"
            v-model="seachVale"
          >
          </el-input>
          <el-select v-model="shen_status" class="marginRight lt" placeholder="请选择通知状态">
            <el-option
        ...
        <el-row :gutter="20" class="goodsindex-list">
          <el-col :span="24">
            ...
            >
              ...
              <el-table-column prop="notice_type_str" label="通知状态"> </el-table-column>
              ...
              <el-table-column label="阅读状态">
                <template slot-scope="scope">
                  ...
                  <div v-else-if="scope.row.state == 2">未读</div>
                  <div v-else>未知</div>
                </template>
              </el-table-column>
...
<script>
import { selling_notice_log } from "../../utils/axios";
export default {
  data() {
    return {
      queryInfo: {
      ...
      edit_tan: false,
      add_tan: false,
      fn1: this.commonJs.Debounce(this.get_selling_notice_log),
      extendList: [],
      seachVale: "",
      loading: false,
      multipleSelection: [],
  ...
  created() {
    this.get_selling_notice_log(); //获取推广列表
  ...
  methods: {
    ...
    get_selling_notice_log(index) {
      ...
      selling_notice_log(request_form)
        .then((res) => {
          if (res.data.err_code == 0) {
            this.loading = false;
