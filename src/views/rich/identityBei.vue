    ...
    <div class="pageItem">
      ...
      <div class="textItem">
        <div class="clearfix" style="margin-bottom: 20px">
          ...
          <el-select v-model="type_status" class="marginRight lt" placeholder="请选择申请身份">
            <el-option
          ...
          <el-button type="primary" class="rt" @click="pick_seach">搜索</el-button>
          <el-button
            type="primary"
            class="rt"
          ...
          <el-col :span="24">
            ...
            >
              ...
              <el-table-column label="状态">
                <template slot-scope="scope">
                  <div v-if="scope.row.state == 1" style="color:#f00f0f;">审核中</div>
                  <div v-else-if="scope.row.state == 2">审核成功</div>
                  <div v-else-if="scope.row.state == 3" style="color:#ee0dc6;">审核拒绝</div>
                  <div v-else-if="scope.row.state == 4" style="color:#ee8f29;">候补中</div>
        ...
        <el-dialog
          ...
          append-to-body
        >
          <div class="info" v-if="types == 1"><s class="s"></s>{{shen_wen.title}}</div>
          <div class="info" v-else-if="types == 2"><s class="s"></s>更换区域</div>
          <div class="info" v-else-if="types == 3 || types == 4"><s class="s"></s>详情</div>
            ...
            <div class="blockBox divs">
              <el-form :model="shen_wen" class="form por clearfix">
                ...
                <div class="clearfix">
                    ...
                    <div class="rt yiban" v-else-if="types == 2 || types == 3 || types == 4">
                        <h2>选择省级区域</h2>
                        <el-form-item
                            label="选择区域:"
                            :label-width="formLabelWidth1"
                            >
                            <!-- <span>{{shen_wen.contact_province}}{{shen_wen.contact_city}}{{shen_wen.contact_area}}</span> -->
                            <span class="poa" style="right:0;top:0;color:#409eff;cursor:pointer;" @click="quEdit('2')">编辑</span>
                        </el-form-item>
                        
                    </div>
                </div>
                <!--  -->
                <div class="clearfix" v-if="(types == 4 || types == 3) && shen_wen.certificate.length > 0">
                    <!-- types -->
                    <div class="lt yiban">
                        <h2>打款凭证</h2>
                        <el-carousel height="150px" style="width: 100%;" :autoplay="false">
                          <el-carousel-item v-for="item in shen_wen.certificate" :key="item">
                              <el-image
                                :src="item"
                                fit="cover"
                        ...
                        <!-- <el-image
                            v-for="(item,index) in shen_wen.certificate" :key="index"
                            :src="item"
                            fit="cover"
                            style="width: 100px; height: 50px;margin-right:20px;margin-bottom:20px;"
                            :preview-src-list="shen_wen.certificate"
                        ></el-image> -->
                    </div>
                </div>
                ...
                <div class="clearfix" v-if="types == 1">
                    ...
                    <div class="rt yiban" style="margin-top:20px" v-if="types == 1">
                      <el-input
                          type="textarea"
...
<script>
import { agent_list,agent_info,get_area,get_city,get_province,agent_update } from "../../utils/axios";
export default {
  data() {
    return {
      queryInfo: {
  ...
  methods: {
    ...
    wen(type) {
      // <!--shen_wen.        -->
      if (this.shen_wen.bank_card_user == ""||this.shen_wen.bank_card == ""||
      this.shen_wen.opening_bank == ""||this.shen_wen.contact_province == ""||
      this.shen_wen.contact_city == ""||this.shen_wen.contact_area == ""||
      this.shen_wen.contact_address == ""||this.shen_wen.contact_phone == "") {
        this.$message({
          showClose: true,
          message: "所有内容必填",
          type: "error",
    ...
    //搜索
    pick_seach() {
      if (this.seachVale == "" && this.seachVale1 == "" && this.time_value == "" && this.type_status == "" && this.shen_status == "") {
        this.$message({
          message: "请填写搜索内容",
          type: "warning",
        });
